...
class build_var_spoof : public zygisk::ModuleBase
{
public:
        ...
        std::apply([&key, &value](auto &&...args)
                   { ((key == std::remove_cvref_t<decltype(args)>::getField() &&
                       (LOGD("Read config: %.*s = %.*s", static_cast<int>(key.size()), key.data(),
                        ...
                        args.value.size() >= value.size() + 1 ? (args.has_value = true,
...
DEVICE_INITIAL_SDK_INT=35
    ...
    std::unique_ptr<FILE, decltype([](auto *f)
                                   { fclose(f); })>
        config{fdopen(cfd, "r")};
...
REGISTER_ZYGISK_MODULE(build_var_spoof)
